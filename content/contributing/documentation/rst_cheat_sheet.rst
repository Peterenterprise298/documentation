...
Use the `icon` markup to add a class name of an icon. There are two icon sets used in the Odoo user
interface: `FontAwesome4 <https://fontawesome.com/v4/icons/>`_ and :doc:`Odoo UI
...
.. list-table::
   ...
   * - The graph view is represented by the :icon:`fa-bar-chart` :guilabel:`(bar chart)` icon. The
...
.. note::
   Both *FontAwesome* and *Odoo UI* icons **do not** need the main class (`fa` or `oi`) to be added
   in conjunction with the specific icon class when used in the :abbr:`RST (reStructuredText)`
   documentation.
